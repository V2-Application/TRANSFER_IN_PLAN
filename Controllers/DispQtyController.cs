...
public class DispQtyController : Controller
{
    ...
    public DispQtyController(IConfiguration config) =>
        _sfConnStr = config.GetConnectionString("Snowflake")!;

    private const string TABLE = "QTY_DISP_QTY";
    ...
    private static DispQty ReadRow(IDataReader r)
    {
        var m = new DispQty
        {
        ...
        for (int w = 1; w <= 48; w++)
        {
            var prop = typeof(DispQty).GetProperty($"Wk{w}");
    ...
    private static object?[] BuildValues(DispQty model)
    {
    ...
    // ── Create GET ───────────────────────────────────────────
    public IActionResult Create() => View(new DispQty());

    // ── Create POST ──────────────────────────────────────────
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Create(DispQty model)
    {
    ...
    // ── Edit POST ────────────────────────────────────────────
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(DispQty model)
    {
        if (!ModelState.IsValid) return View(model);
        await using var conn = await SnowflakeCrudHelper.OpenAsync(_sfConnStr);
