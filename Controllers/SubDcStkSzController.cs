...
public class SubDcStkSzController : Controller
{
    ...
    public SubDcStkSzController(PlanningDbContext context) => _context = context;
    ...
    {
        var query = _context.SubDcStkSzs.AsQueryable();
        ...
        ViewBag.RdcCodes = await _context.SubDcStkSzs.Select(x => x.RdcCd).Distinct().OrderBy(x => x).ToListAsync();
        ViewBag.MajCats = await _context.SubDcStkSzs.Select(x => x.MajCat).Distinct().OrderBy(x => x).ToListAsync();
        ViewBag.SubValues = await _context.SubDcStkSzs.Select(x => x.SubValue).Distinct().OrderBy(x => x).ToListAsync();
        ViewBag.TotalRows = await _context.SubDcStkSzs.CountAsync();
        ViewBag.TotalRdcs = await _context.SubDcStkSzs.Select(x => x.RdcCd).Distinct().CountAsync();
        ViewBag.TotalCats = await _context.SubDcStkSzs.Select(x => x.MajCat).Distinct().CountAsync();
    ...
    public async Task<IActionResult> Create(SubDcStkSz model)
    {
        ...
        _context.SubDcStkSzs.Add(model);
    ...
    {
        var item = await _context.SubDcStkSzs.FindAsync(id);
    ...
    {
        var item = await _context.SubDcStkSzs.FindAsync(id);
        if (item != null) { _context.SubDcStkSzs.Remove(item); await _context.SaveChangesAsync(); }
