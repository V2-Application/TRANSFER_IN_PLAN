...
public class BulkUploadController : Controller
{
    ...
    public IActionResult DownloadSample(string table)
    {
        ...
        for (int i = 0; i < sample.Length; i++)
            ws.Cells[2, i + 1].Value = sample[i];

        ws.Cells[ws.Dimension.Address].AutoFitColumns();

        var bytes = package.GetAsByteArray();
        _logger.LogInformation("Sample Excel downloaded for table: {Table}", table);
        return File(bytes,
    ...
    public async Task<IActionResult> Upload(IFormFile file, string table, string mode)
    {
        ...
        if (ext != ".xlsx" && ext != ".xls")
        {
            TempData["ErrorMessage"] = "Only Excel files (.xlsx / .xls) are supported.";
            return RedirectToAction(nameof(Index));
        }
        ...
        try
        {
            ...
            {
                TempData["ErrorMessage"] = "The uploaded Excel file is empty or invalid.";
                return RedirectToAction(nameof(Index));
            }
            ...
            if (mode == "Replace")
            {
                ...
                _logger.LogInformation("BulkUpload Replace mode: cleared table {Table}", table);
            }

            for (int r = 2; r <= rows; r++)  // row 1 = header
            {
                bool ok = await InsertRow(table, ws, r);
                if (ok) inserted++; else skipped++;
            }

            _logger.LogInformation("BulkUpload {Mode} on {Table}: {Inserted} inserted, {Skipped} skipped.", mode, table, inserted, skipped);
            TempData["SuccessMessage"] = $"Upload complete — {inserted} records inserted, {skipped} skipped (empty/invalid rows).";
        }
        catch (Exception ex)
        {
    ...
    private async Task ClearTable(string table)
    {
        switch (table)
        {
            case "WeekCalendar": _context.WeekCalendars.RemoveRange(_context.WeekCalendars); break;
            case "StoreMaster":  _context.StoreMasters.RemoveRange(_context.StoreMasters); break;
            case "BinCapacity":  _context.BinCapacities.RemoveRange(_context.BinCapacities); break;
            case "SaleQty":      _context.SaleQties.RemoveRange(_context.SaleQties); break;
            case "DispQty":      _context.DispQties.RemoveRange(_context.DispQties); break;
            case "StoreStock":   _context.StoreStocks.RemoveRange(_context.StoreStocks); break;
            case "DcStock":      _context.DcStocks.RemoveRange(_context.DcStocks); break;
            case "DelPending":   _context.DelPendings.RemoveRange(_context.DelPendings); break;
        }
    ...
    private decimal? Dec(ExcelWorksheet ws, int row, int col)
        => decimal.TryParse(Cell(ws, row, col), out var d) ? d : null;

    private int? Int(ExcelWorksheet ws, int row, int col)
        => int.TryParse(Cell(ws, row, col), out var i) ? i : null;

    private DateTime? Dt(ExcelWorksheet ws, int row, int col)
    {
    ...
    private async Task<bool> InsertRow(string table, ExcelWorksheet ws, int r)
    {
        try
        {
            switch (table)
            {
                case "WeekCalendar":
                {
                    if (string.IsNullOrEmpty(Cell(ws, r, 1))) return false;
                    var rec = new WeekCalendar
                    {
                        WeekId  = Int(ws, r, 1) ?? 0,
                        WeekSeq = Int(ws, r, 2) ?? 0,
                        FyWeek  = Int(ws, r, 3) ?? 0,
                        FyYear  = Int(ws, r, 4) ?? 0,
                        CalYear = Int(ws, r, 5) ?? 0,
                        YearWeek = Cell(ws, r, 6),
                        WkStDt  = Dt(ws, r, 7),
                        WkEndDt = Dt(ws, r, 8)
                    };
                    _context.WeekCalendars.Add(rec);
                    break;
                }
                case "StoreMaster":
                {
                    if (string.IsNullOrEmpty(Cell(ws, r, 1))) return false;
                    var rec = new StoreMaster
                    {
                        StCd      = Cell(ws, r, 1),
                        StNm      = Cell(ws, r, 2),
                        RdcCd     = Cell(ws, r, 3),
                        ...
                        HubCd     = Cell(ws, r, 5),
                        HubNm     = Cell(ws, r, 6),
                        Status    = Cell(ws, r, 7) ?? "Active",
                        GridStSts = Cell(ws, r, 8),
                        OpDate    = Dt(ws, r, 9),
                        Area      = Cell(ws, r, 10),
                        State     = Cell(ws, r, 11),
                        RefState  = Cell(ws, r, 12),
                        SaleGrp   = Cell(ws, r, 13),
                        RefStCd   = Cell(ws, r, 14),
                        RefStNm   = Cell(ws, r, 15),
                        RefGrpNew = Cell(ws, r, 16),
                        RefGrpOld = Cell(ws, r, 17),
                        Date      = Dt(ws, r, 18)
                    };
                    _context.StoreMasters.Add(rec);
                    break;
                }
                case "BinCapacity":
                {
                    if (string.IsNullOrEmpty(Cell(ws, r, 1))) return false;
                    var rec = new BinCapacity
                    {
                        MajCat      = Cell(ws, r, 1),
                        BinCapDcTeam = Dec(ws, r, 2),
                        BinCap      = Dec(ws, r, 3)
                    };
                    _context.BinCapacities.Add(rec);
                    break;
                }
                case "SaleQty":
                {
                ...
                case "DispQty":
                {
                    ...
                    _context.DispQties.Add(rec);
                    break;
                }
                case "StoreStock":
                {
                    if (string.IsNullOrEmpty(Cell(ws, r, 1))) return false;
                    var rec = new StoreStock
                    {
                        StCd   = Cell(ws, r, 1),
                        MajCat = Cell(ws, r, 2),
                        StkQty = Dec(ws, r, 3),
                ...
                case "DcStock":
                {
                    ...
                    _context.DcStocks.Add(rec);
                    break;
                }
                case "DelPending":
                {
                    if (string.IsNullOrEmpty(Cell(ws, r, 1))) return false;
                    var rec = new DelPending
                    {
                        RdcCd    = Cell(ws, r, 1),
                        MajCat   = Cell(ws, r, 2),
                        DelPendQ = Dec(ws, r, 3),
        ...
        {
            _logger.LogWarning(ex, "Skipped row {Row} in {Table} due to error.", r, table);
            // Detach failed tracked entities so EF doesn't block next inserts
