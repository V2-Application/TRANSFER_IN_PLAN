...
public class SubStStkMvgrController : Controller
{
    ...
    public SubStStkMvgrController(IConfiguration config) =>
        _sfConnStr = config.GetConnectionString("Snowflake")!;

    private const string TABLE = "SUB_ST_STK_MVGR";
    ...
    private static SubStStkMvgr ReadRow(IDataReader r) => new()
    {
    ...
    public async Task<IActionResult> Create(SubStStkMvgr model)
    {
    ...
    // ── Edit POST ────────────────────────────────────────────
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(SubStStkMvgr model)
    {
        if (!ModelState.IsValid) return View(model);
        await using var conn = await SnowflakeCrudHelper.OpenAsync(_sfConnStr);
    ...
    public async Task<IActionResult> ExportCsv(string? stCd, string? majCat)
    {
        ...
        return File(Encoding.UTF8.GetBytes(sb.ToString()), "text/csv", "StoreStock_Mvgr.csv");
    }
}
